...
expr <- read.csv(params$expression_file, row.names=1, check.names=F)
...
goGeneSet <- readGMT(params$GOGeneset, normalized=params$GeneSet_Name_Normalized)
info(logger, "GO Gene Sets:\t", params$GOGeneset)
info(logger, paste("GO共有", length(goGeneSet), "个基因集"))

pathwayGeneSet <- readGMT(params$PWGeneset, normalized=params$GeneSet_Name_Normalized)
